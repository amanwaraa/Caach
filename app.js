...
function safeCreateIcons() {
  try {
    ...
    console.warn("icons skipped", error);
  }
}
...
function bindBaseEvents() {
  ...
  qs("licenseKeyIconBtn")?.addEventListener("click", async () => {
    const input = qs("licenseKeyInput");
    input?.focus();
    try {
      if (input && !input.value && navigator.clipboard?.readText) {
        const text = await navigator.clipboard.readText();
        if (text) input.value = text.trim();
      }
      if (input && input.value) showToast("تم وضع المفتاح، اضغط دخول", "success");
    } catch {
      if (input) showToast("الصق المفتاح يدويًا ثم اضغط دخول", "info");
    }
  });
...
function setLoginBusy(isBusy) {
  ...
  btn.innerText = isBusy ? "جاري الدخول..." : "دخول";
}
...
function withTimeout(promise, ms = 12000, message = "انتهت مهلة الاتصال بقاعدة البيانات") {
  let timer;
...
function attachRealtimeListeners() {
  ...
  onValue(productsListenerRef, async snap => {
    const items = snap.exists() ? Object.entries(snap.val() || {}).map(([key, value]) => ({ ...(value || {}), id: String(value?.id || key), __key: key, firebaseKey: key })) : [];
...
async function handleLicenseLogin() {
  if (handleLicenseLogin._busy) return;
  handleLicenseLogin._busy = true;
  setLoginBusy(true);
  setLoginError("");
  try {
    ...
    if (!key) {
      setLoginError("يرجى إدخال المفتاح");
      return;
    }
    ...
    const licensePath = `${pathLicenses()}/${sanitizeKey(key)}`;
    let snap = await withTimeout(get(ref(db, licensePath)), 12000, "تعذر الاتصال بقاعدة البيانات، افحص الإنترنت وقواعد Firebase");
    if (!snap.exists()) {
      const clientSnap = await withTimeout(get(ref(db, `${PREFIX}_clients/${sanitizeKey(key)}`)), 12000, "تعذر الاتصال بقاعدة البيانات");
      if (clientSnap.exists()) {
        await set(ref(db, licensePath), {
          key,
          status: "active",
          ...
          createdAt: new Date().toISOString(),
          repairedFromClientPath: true
        });
        snap = await get(ref(db, licensePath));
      } else {
        showLogin("المفتاح غير موجود. أنشئه من أدمن مفاتيح الكاشير أولاً ثم جرّب الدخول.");
        return;
      }
    }
    ...
    const previousSession = getLocalSession();
    const sameSavedKey = previousSession && String(previousSession.key || "") === String(key || "");
    if (maxLogins !== null && usedLogins >= maxLogins && !sameSavedKey) {
    ...
    await withTimeout(update(ref(db, licensePath), {
      startedAt,
      expiresAt,
      usedLogins: sameSavedKey ? usedLogins : usedLogins + 1,
      lastLoginAt: new Date().toISOString()
    }), 12000, "تعذر تحديث بيانات المفتاح");
    ...
    hideLoaderImmediate();
    showLogin(error?.message || "فشل تسجيل الدخول، تأكد من المفتاح والإنترنت وقواعد Firebase");
  } finally {
    setLoginBusy(false);
    handleLicenseLogin._busy = false;
  }
}
...
function findProductForStockLine(products, item) {
  const cleanValue = v => String(v || "").trim();
  const wantedId = cleanValue(item?.id || item?.productId || "");
  const wantedCode = cleanValue(item?.code || item?.barcode || item?.cashierBarcode || "");
  const wantedKey = cleanValue(item?.cashierProductKey || item?.firebaseKey || item?.__key || item?.productKey || "");
  const wantedStoreId = cleanValue(item?.linkedStoreProductId || item?.storeProductId || item?.productId || "");
  return products.find(x => wantedKey && [x.__key, x.firebaseKey, x.cashierProductKey, x.id].map(cleanValue).includes(wantedKey))
    || products.find(x => wantedId && [x.id, x.__key, x.firebaseKey, x.cashierProductId].map(cleanValue).includes(wantedId))
    || products.find(x => wantedStoreId && cleanValue(x.linkedStoreProductId) === wantedStoreId)
    || products.find(x => wantedCode && [x.code, x.barcode, x.cashierBarcode].map(cleanValue).includes(wantedCode))
    || null;
}

function mutateProductStockForLine(product, item, direction) {
  const matrix = safeVariantMatrix(product.variantMatrix, product.variants, product.stock);
  const row = findMatrixRow(matrix, item.selectedSize || item.selectedVariant || item.size || "");
  const color = findMatrixColor(row, item.selectedColorKey || item.colorKey, item.selectedColorName || item.colorName, item.selectedColorCode || item.colorCode);
  const qtyDelta = Number(direction || 0) * Number(item.qty || item.quantity || 0);

  if (!row || !color) {
    throw new Error(`لم يتم العثور على مخزون ${item.name || product.name || "منتج"} / ${item.selectedSize || item.selectedVariant || item.size || "مقاس"} / ${item.selectedColorName || item.colorName || "لون"}`);
  }

  const currentStock = Math.max(0, Number(color.stock || 0));
  const nextStock = currentStock + qtyDelta;
  if (nextStock < 0) {
    throw new Error(`المخزون غير كافٍ: ${item.name || product.name || "منتج"}. المتاح ${currentStock} والمطلوب ${Math.abs(qtyDelta)}`);
  }

  color.stock = nextStock;
  return {
    ...product,
    variantMatrix: matrix,
    variants: flattenVariantsFromMatrix(matrix),
...
function expandInvoiceItemsToStockLines(items) {
  const groups = new Map();
  for (const item of items || []) {
    const units = Array.isArray(item.unitSelections) ? item.unitSelections : [];
    const sourceLines = units.length
      ? units.map(unit => ({
          ...item,
          selectedSize: unit.size || item.selectedSize || item.selectedVariant || "بدون مقاس",
          selectedVariant: unit.size || item.selectedVariant || item.selectedSize || "بدون مقاس",
          selectedColorKey: unit.colorKey || unit.selectedColorKey || item.selectedColorKey || "",
          selectedColorName: unit.colorName || unit.selectedColorName || item.selectedColorName || "",
          selectedColorCode: unit.colorCode || unit.selectedColorCode || item.selectedColorCode || "",
          qty: 1
        }))
    ...
    for (const line of sourceLines) {
      const key = [
        ...
        line.code || line.barcode || line.cashierBarcode || "",
        line.selectedSize || line.selectedVariant || line.size || "",
        line.selectedColorKey || line.colorKey || "",
        line.selectedColorName || line.colorName || "",
        line.selectedColorCode || line.colorCode || ""
      ].map(v => String(v || "")).join("__");
      if (!groups.has(key)) groups.set(key, { ...line, qty: 0 });
      groups.get(key).qty += Number(line.qty || 1);
    }
  }
...
async function applyStockChange(items, direction) {
  ...
  for (const item of expandInvoiceItemsToStockLines(items)) {
    ...
    if (!p) {
      const generatedId = String(item.cashierProductKey || item.id || item.productId || item.code || item.barcode || `auto_${Date.now()}`).trim();
      p = {
        id: generatedId,
        __key: String(item.cashierProductKey || item.firebaseKey || generatedId),
        firebaseKey: String(item.cashierProductKey || item.firebaseKey || generatedId),
        linkedStoreProductId: String(item.linkedStoreProductId || item.productId || ""),
        name: item.name || "منتج متجر",
        code: item.code || item.barcode || item.cashierBarcode || "",
        barcode: item.barcode || item.code || item.cashierBarcode || "",
        cashierBarcode: item.cashierBarcode || item.barcode || item.code || "",
        cost: Number(item.cost || 0),
        price: Number(item.price || 0),
        stock: Number(item.stock || 0),
        variants: Array.isArray(item.variants) ? item.variants : [],
        variantMatrix: safeVariantMatrix(item.variantMatrix, item.variants, item.stock),
        storeId: item.storeId || currentStoreId || "default",
        createdAt: new Date().toISOString()
      };
      await saveEntity("products", p.__key || p.id, p);
      products.push(p);
    }

    if (onlineSync) {
      const productKey = String(p.__key || p.firebaseKey || p.id || "").trim();
      const productPath = `${pathClientProducts()}/${productKey}`;
      let failureMessage = "";
      const result = await transaction(ref(db, productPath), current => {
        try {
          const base = current ? { ...current, id: String(current?.id || p.id || productKey), __key: productKey, firebaseKey: productKey } : p;
          const updated = mutateProductStockForLine(base, item, direction);
          return { ...updated, id: String(updated.id || p.id || productKey), __key: productKey, firebaseKey: productKey };
        } catch (error) {
          failureMessage = error?.message || "تعذر تعديل المخزون";
          return;
        }
      });

      if (!result.committed) {
        throw new Error(failureMessage || "تعذر تعديل المخزون");
      }
...
async function validateCartAgainstStock() {
  ...
  for (const item of cart) {
    const product = findProductForStockLine(products, item);
    if (!product) {
  ...
  try {
    ...
    if (editingInvoiceId) {
      ...
      try {
        ...
        updateCreateInvoiceButton();
        clearInvoiceEditor();
        showToast("تم حفظ تعديل الفاتورة و تحديث المخزون", "success");
...
async function editInvoice(id) {
  ...
  if (inv.source === "store") {
    alert("فاتورة المتجر مرتبطة بطلب متجر، احذفها أو راجع الطلب من لوحة المتجر بدلاً من تعديلها من الكاشير.");
    return;
  }
...
async function deleteInvoice(id) {
  if (!confirm("حذف الفاتورة؟ سيتم إرجاع الكميات للمخزون في الكاشير والمتجر.")) return;

  await showLoader("جاري حذف الفاتورة وإرجاع المخزون...");
  try {
    const inv = await getEntity("invoices", id);
    if (!inv) return;
    ...
    if (inv.source === "store" && inv.storeOrderId && isOnline()) {
      try {
        await update(ref(db, `${STORE_ORDERS_PATH}/${inv.storeOrderId}`), {
          inventoryDeducted: false,
          inventoryDeductedAt: null,
          cashierInventoryDeducted: false,
          cashierInventoryDeductedAt: null,
          cashierInvoiceCreated: false,
          cashierInvoiceDeletedAt: Date.now(),
          stockReturnedFromCashierInvoiceDelete: true,
          updatedAt: Date.now()
        });
      } catch (error) {
        console.warn("Store order flags restore skipped", error);
      }
    }
    ...
    showToast("تم حذف الفاتورة وإرجاع المخزون", "success");
    await renderInvoices();
    if (!qs("tab-reports")?.classList.contains("hidden")) await renderReports();
  } catch (error) {
    console.error("Delete invoice failed", error);
    hideLoaderImmediate();
    alert(error?.message || "فشل حذف الفاتورة أو إرجاع المخزون");
  }
}
...
async function syncCashierProductToLinkedStore(product) {
  try {
    ...
    const matrix = safeVariantMatrix(product.variantMatrix, product.variants, product.stock);
    const productKey = String(product.__key || product.firebaseKey || product.id || "").trim();
    const updatePayload = {
      cashierProductId: product.id || productKey,
      cashierProductKey: productKey,
    ...
    Object.entries(snap.val() || {}).forEach(([storeProductId, storeProduct]) => {
      ...
      const linkedKey = String(storeProduct?.cashierProductKey || "").trim();
      if (linkedCode === code || linkedId === String(product.id || "") || linkedId === productKey || linkedKey === productKey || linkedKey === String(product.id || "")) {
...
async function syncCloudToOffline() {
  ...
  const stores = storesSnap.exists() ? Object.entries(storesSnap.val() || {}).map(([id, v]) => ({ ...(v || {}), id })) : [];
  const products = productsSnap.exists() ? Object.entries(productsSnap.val() || {}).map(([id, v]) => ({ ...(v || {}), id: String(v?.id || id), __key: id, firebaseKey: id })) : [];
  const invoices = invoicesSnap.exists() ? Object.entries(invoicesSnap.val() || {}).map(([id, v]) => ({ ...(v || {}), id: String(v?.id || id), __key: id, firebaseKey: id })) : [];
  const purchases = purchasesSnap.exists() ? Object.entries(purchasesSnap.val() || {}).map(([id, v]) => ({ ...(v || {}), id })) : [];
